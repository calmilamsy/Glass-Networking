...
public interface GlassPacketListener {
    ...
    default void registerGlassPacket(String fullId, BiConsumer<GlassPacket, NetworkHandler> applicator, boolean clientBound, boolean serverBound) {
        GlassNetworking.PACKET_LISTENERS.computeIfPresent(fullId, (id, applicator2) -> {throw new RuntimeException("Packet ID \"" + id + "\" already exists!");});

        GlassNetworking.PACKET_LISTENERS.put(fullId, applicator);
        if (clientBound) {
